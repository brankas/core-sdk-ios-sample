...
class ViewController: UIViewController, CheckDelegate {
    ...
    @IBAction func onCheckout(_ sender: Any) {
        ...
        let request = BalanceTapRequest(country: countryCode, bankCodes: getCheckedBankCodes(), externalId: tfExternalId.text ?? "", successURL: tfSuccessURL.text ?? "", failURL: tfFailURL.text ?? "", organizationName: tfOrganizationName.text ?? "", redirectDuration: 60, browserMode: .WebView, dismissAlert: nil, isAutoConsent: swAutoConsent.isOn, useRememberMe: swUseRememberMe.isOn)

        BalanceTapSF.shared.initialize(apiKey: Constants.API_KEY, certPath: nil, isDebug: false)

        do {
            try BalanceTapSF.shared.checkout(balanceTapRequest: request, vc: self, closure: { data, err in
    ...
    private func getEnabledBanks() {
        ...
        BalanceTapSF.shared.initialize(apiKey: Constants.API_KEY, certPath: nil, isDebug: false)
        BalanceTapSF.shared.getEnabledBanks(country: countryCode) { data, err in
            self.vProgressing.isHidden = true
    ...
    private func setupBanks(stackView: UIStackView, banks: [BalanceBank]) {
    ...
    private func getCheckedBankCodes() -> [BalanceBankCode] {
        var list: [BalanceBankCode] = []
    ...
    private func setResult(data: Any?, error: String?) {
        if let str = data as? String {
            if let err = error {
                showAlert(message: "Balance ID: \(str)\nError: \(err)")
            }
            else {
                showAccountList(balanceId: str, message: "")
            }
            ...
            showAccountList(balanceId: balanceId, message: message)
        } else {
            if let err = error {
